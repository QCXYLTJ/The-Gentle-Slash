        ...
        const skilltihuan = function () {
            ...
            lib.skill.boss_biantian = {
                ...
                content() {
                    ...
                    ('step 1');
                    let targets = [], players = game.players;
                    if (result.color == 'red') {
            ...
            get() {
                return function () {
                    ...
                    next.hasNature = function (nature) {
                        ...
                        const natures = get.natureList(nature), naturesx = get.natureList(this.nature);
                        if (nature == 'linked') {
    ...
    const config = function () {
        ...
        const css = function () {
            ...
            if (QQQ.config.文字闪烁) {
                ...
                for (let i = 1; i <= 20; i++) {
                    const rand1 = Math.floor(Math.random() * 255), rand2 = Math.floor(Math.random() * 255), rand3 = Math.floor(Math.random() * 255);
            ...
            skill: {
                ...
                QQQ_qingshi: {
                    ...
                    async content(event, trigger, player) {
                        ...
                        if (color == 'red') {
                            const {
                                result: { targets },
                            } = await player.chooseTarget('与其依次视为使用一张【树上开花】,该角色获得〖优游〗直到你下个出牌阶段开始', (c, p, t) => t != p).set('ai', (t) => get.attitude(player, t));
                            if (targets && targets[0]) {
                ...
                QQQ_wuzhiquanxinjian: {
                    ...
                    async content(event, trigger, player) {
                        for (const card of trigger.cards) {
                            ...
                            const {
                                result: { targets },
                            } = await player.chooseTarget(`选择第${player.storage.QQQ_wuzhiquanxinjian}剑的目标`, (c, p, t) => t != player).set('ai', (t) => -get.attitude(t, player));
                            if (targets && targets[0]) {
                ...
                QQQ_kuangzheng: {
                    ...
                    async content(event, trigger, player) {
                        const {
                            result: { targets },
                        } = await player.chooseTarget('对这些角色使用一张【五谷丰登】', [1, game.players.length]).set('ai', (t) => get.attitude(player, t));
                        if (targets && targets[0]) {
                        ...
                        1: {
                            ...
                            async content(event, trigger, player) {
                                ...
                                while (count-- > 0) {
                                    ...
                                    const {
                                        result: { links },
                                    } = await player.chooseButton(['选择使用其中任意牌,并摸没有被使用的牌数的牌', cards], [1, cards.length]).set('ai', (button) => number0(player.getUseValue(button.link, true, true)) - 6);
                                    if (links && links[0]) {
                                        for (const i of links) {
                                            cards.remove(i);
                ...
                QQQ_qiaobian: {
                    ...
                    async content(event, trigger, player) {
                        ...
                        const {
                            result: { cards },
                        } = await player.chooseCard('he', '将一张牌当所有花色均失效的【乐不思蜀】使用', true).set('ai', (c) => 6 - get.value(c));
                        if (cards && cards[0]) {
                            const {
                                result: { targets },
                            } = await player.chooseTarget('选择乐不思蜀的目标', (c, p, t) => p != t).set('ai', (t) => -get.attitude(player, t));
                            if (targets && targets[0]) {
